...
# Load shapefiles with state boundaries and USDA hardiness/growing zones ------#
...
# Download and format NPN data for priority plant species ---------------------#
  ...
  count(spp, is.na(species_id), priority)
    ...
    # 41 spp remain (8 priority 1; 13 priority 2; 6 priority 3; 14 priority 4)
  
  # Remove species that don't have an NPN species_id
  spp <- filter(spp, !is.na(species_id))
...
# Clean up phenophase status and state data -----------------------------------#
  ...
  # leaving missing values for the moment). Removing HI and AK observations 
  # since phenology is likely to be very different in those places.
  df <- df %>%
  ...
  # Check if any coordinates are problematic otherwise, using the political 
  # boundaries shapefile to identify state by lat/lon
  locs <- df %>%
    dplyr::select(site_id, latitude, longitude) %>%
    distinct()
  locsv <- vect(locs, geom = c("longitude", "latitude"), crs = crs(states))
  locs <- locs %>% 
    mutate(STUSPS = terra::extract(states, locsv)[,"STUSPS"])
  df <- df %>%
    left_join(select(locs, site_id, STUSPS), by = "site_id")
  # Look at instances where state in NPN database doesn't match what was
  # extracted from polygon shapefile
  df %>%
    filter(state != STUSPS) %>%
    count(state, STUSPS)
    # Removing instances where the states don't match up, EXCEPT where the 
    # mismatched states are geographically adjacent (probably because the site
    # is very close to the state boundary [eg, TN, NC])
  df <- df %>% 
    filter(!(state == "MN" & STUSPS == "CA")) %>%
    filter(!(state == "ND" & STUSPS == "VA")) %>%
    filter(!(state == "NM" & STUSPS == "SC")) %>%
    select(-STUSPS)
...
# Same plant, phenophase, on the same date - different observers
  ...
  sum(datep$n_obs > 1) / nrow(datep) * 100
    # 6.8% of plant-phenophase-date combinations are associated with 
  ...
  # UNCO BIO (which they've had other issues with). NY Botanical Garden and 
  # Mohonk Preserve are reliable groups so keep their data in.
  df <- df %>%
    filter(!grepl("Oak Hill", site_name)) %>%
    filter(!grepl("UNCO", partner_group))
  
  # Recreate summary dataframe and recalcuate the extent to which we have 
  # multiple observations of same plant on same date
  datep <- df %>%
    group_by(individual_id, site_id, site_name, partner_group, 
             observation_date, phenophase_id) %>%
    summarize(n_obs = n(),
  ...
  # For now will use Option 1 even though Option 2 is most similar to the 
  # approach that was used previously. We'll use a sequential process to select 
  # a single observation:
  # 1) select the complete observation (ideally, status and intensity values for
  #    both phenophases). See obs_rank dataframe below for ranking of 
  #    information 
  ...
  obs <- obs %>%
    ...
    # observations of that plant phenophase on that date:
  ...
  # on observation rank (how much information was recorded), and then based on 
...
# Calculate estimated number of open flowers ----------------------------------#
  ...
  # number of open flowers as the product of the number of flowers (midpoint) 
  # and proportion of flowers open (midpoint)
  obs2 <- obs2 %>%
    mutate(num_open = if_else(is.na(midpoint_fl) | is.na(midpoint_fo), 
                              NA, midpoint_fl * midpoint_fo))

# Attach USDA zones to status/intensity data ----------------------------------#

  locs <- obs2 %>%
    dplyr::select(site_id, latitude, longitude) %>%
    distinct()
  locsv <- vect(locs, geom = c("longitude", "latitude"), crs = crs(states))
  # Note: using intersect() rather than extract() here since it seems to be 
  # faster with the zones polygon layer
  locs <- locs %>% 
    mutate(zone = as.data.frame(terra::intersect(locsv, zones))[,"zone"]) %>%
    mutate(zone = str_pad(zone, width = 3, side = "left", pad = 0))
  
  # Attach to status/intensity data
  obs2 <- obs2 %>%
    left_join(select(locs, site_id, zone), by = "site_id") %>%
    relocate(zone, .after = "state")

---
title: "Time to Restore - 2023-2026 data"
...
data_filename <- "data/ttr-data-2024apr2026.csv"
...
if (!file.exists(data_filename) | update_data == TRUE) {
  ...
  # Download leafing data for milkweeds in 2024-2026
  milkweeds <- spp_list %>%
...
status <- read.csv(data_filename) %>%
  ...
  mutate(wy = case_when(
    obsdate < "2024-10-01" ~ 2024,
    obsdate < "2025-10-01" ~ 2025,
    .default = 2026
  ))
```

For most summaries in this report, we used all observations from 1 October 2023 - 26 April 2026. Occasionally we compared current water-year data (observations submitted on or after 1 October 2025) with data collected during the previous water years (October 2023 - September 2024; October 2024 - September 2025), while keeping in mind that we are only partially through the current water year.
...
There were `r nrow(int_no)` instances since October 2023 where an observer did not report "yes" for a particular phenohpase, but did report an intensity value.

```{r, include = nrow(int_no) > 0}
...
intensitycat_high %>%
  select(common_name, php, intensity_value, nhigh, nplants) %>%
  kable(col.names = c("Species", "Phenophase", "Intensity category", 
                      "No. observations", "No. plants"),
        align = c("l", "c", "r", "c", "c"))
```
...
flower_probs_short <- flower_probs %>%
  select(common_name, n, n_flower0_openNot0, n_flowerNA_open1)

knitr::kable(flower_probs_short,
             col.names = c("Species", "No. observations", "Flowers:no AND Open flowers:yes/?",
...
fruit_probs_short <- fruit_probs %>%
  select(common_name, n, n_fruit0_ripeNot0, n_fruitNA_ripe1)

knitr::kable(fruit_probs_short,
             col.names = c("Species", "No. observations", "Fruit:no AND Ripe fruit:yes/?",
...
# water year (Oct 2023 to present)
status <- status %>%
...
locs_by_state_int <- locs %>%
  ...
  summarize(nspp_per_site = round(mean(nspp), 2),
            ...
            nobs_per_site = round(mean(nobs)),
            nrecords_per_site = round(mean(nrecords))) %>%
  data.frame()
...
map_species <- ggplot(data = ttr_counties, aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
...
map_records <- ggplot(data = ttr_counties, aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
  geom_polygon(color = "gray", fill = NA, linewidth = 0.2) + 
  geom_polygon(data = ttr_states, color = "black", fill = NA) +
  geom_point(data = locs, aes(x = lon, y = lat, group = NA, size = nrecords), 
             color = "blue") +
  labs(size = "No. records") +
  guides(size = guide_legend(position = "inside")) +
  theme_bw() +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(), axis.title.y = element_blank(), 
...
map_spp_file <- "ttr-apr2026-figs/ttr-2026-map-species.png"
ggsave(filename = map_spp_file,
       plot = map_species,
       width = 6,
       height = 4,
       units = "in",
...
map_records_file <- "ttr-apr2026-figs/ttr-2026-map-records.png"
ggsave(filename = map_records_file,
       plot = map_records,
       width = 6,
       height = 4,
       units = "in",
...
# Create state-level static maps
  ...
  ggsave("ttr-apr2026-figs/LA-map-species.png",
         la_map_spp,
         height = 6,
         width = (6 * la_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/LA-map-records.png",
         la_map_rec,
         height = 6,
         width = (6 * la_ar) + 0.5,
         units = "in",
  ...
  ggsave("ttr-apr2026-figs/NM-map-species.png",
         nm_map_spp,
         height = 6,
         width = (6 * nm_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/NM-map-records.png",
         nm_map_rec,
         height = 6,
         width = (6 * nm_ar) + 0.5,
         units = "in",
  ...
  ggsave("ttr-apr2026-figs/OK-map-species.png",
         ok_map_spp,
         height = 6,
         width = (6 * ok_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/OK-map-records.png",
         ok_map_rec,
         height = 6,
         width = (6 * ok_ar) + 0.5,
         units = "in",
  ...
  ggsave("ttr-apr2026-figs/TX-map-species.png",
         tx_map_spp,
         height = 6,
         width = (6 * tx_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/TX-map-records.png",
         tx_map_rec,
         height = 6,
         width = (6 * tx_ar) + 0.5,
         units = "in",
  ...
  # Dallas area
  dallas_bounds <- c(-98, 32, -96, 34)
  dallas <- matrix(dallas_bounds, 2, byrow = TRUE) %>%
    st_multipoint() %>%
    st_sfc(crs = 4269) %>%
    st_transform(3857) %>%
    get_tiles(provider = "OpenStreetMap", zoom = 9, crop = TRUE)
  box <- c(xmin = dallas_bounds[1], ymin = dallas_bounds[2],
           xmax = dallas_bounds[3], ymax = dallas_bounds[4])
  locs_dallas <- st_crop(locs_tx, box)

  dallas_map_spp <- ggplot() +
    geom_spatraster_rgb(data = dallas, maxcell = Inf) +
    geom_sf(data = locs_dallas,
            aes(size = nspp),
            show.legend = "point",
            color = "blue",
            alpha = 0.5) +
    scale_size_continuous(range = c(2, 5)) +
    labs(size = "No. species") +
    coord_sf(crs = 3857, expand = FALSE, ylim = st_bbox(dallas)[c(2, 4)]) +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank())
  
  dallas_map_rec <- ggplot() +
    geom_spatraster_rgb(data = dallas, maxcell = Inf) +
    geom_sf(data = locs_dallas,
            aes(size = nrecords),
            show.legend = "point",
  ...
  dallas_ar <- (dallas_bounds[3] - dallas_bounds[1]) / (dallas_bounds[4] - dallas_bounds[2])
  ggsave("ttr-apr2026-figs/TX-Dallas-map-species.png",
         dallas_map_spp,
         height = 6,
         width = (6 * dallas_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/TX-Dallas-map-records.png",
         dallas_map_rec,
         height = 6,
         width = (6 * dallas_ar) + 0.5,
         units = "in",
  ...
  austin_bounds <- c(-99, 29, -97, 31)
  austin <- matrix(austin_bounds, 2, byrow = TRUE) %>%
    st_multipoint() %>%
    st_sfc(crs = 4269) %>%
    st_transform(3857) %>%
    get_tiles(provider = "OpenStreetMap", zoom = 9, crop = TRUE)
  box <- c(xmin = austin_bounds[1], ymin = austin_bounds[2],
           xmax = austin_bounds[3], ymax = austin_bounds[4])
  locs_austin <- st_crop(locs_tx, box)

  austin_map_spp <- ggplot() +
    geom_spatraster_rgb(data = austin, maxcell = Inf) +
    geom_sf(data = locs_austin,
            aes(size = nspp),
            show.legend = "point",
            color = "blue",
            alpha = 0.5) +
    scale_size_continuous(range = c(2, 5)) +
    labs(size = "No. species") +
    coord_sf(crs = 3857, expand = FALSE, ylim = st_bbox(austin)[c(2, 4)]) +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank())
  
  austin_map_rec <- ggplot() +
    geom_spatraster_rgb(data = austin, maxcell = Inf) +
    geom_sf(data = locs_austin,
            aes(size = nrecords),
            show.legend = "point",
  ...
  austin_ar <- (austin_bounds[3] - austin_bounds[1]) / (austin_bounds[4] - austin_bounds[2])
  ggsave("ttr-apr2026-figs/TX-austin-map-species.png",
         austin_map_spp,
         height = 6,
         width = (6 * austin_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/TX-austin-map-records.png",
         austin_map_rec,
         height = 6,
         width = (6 * austin_ar) + 0.5,
         units = "in",
  ...
  # Houston area
  houston_bounds <- c(-96, 29, -94.5, 30.5)
  houston <- matrix(houston_bounds, 2, byrow = TRUE) %>%
    st_multipoint() %>%
    st_sfc(crs = 4269) %>%
    st_transform(3857) %>%
    get_tiles(provider = "OpenStreetMap", zoom = 9, crop = TRUE)
  box <- c(xmin = houston_bounds[1], ymin = houston_bounds[2],
           xmax = houston_bounds[3], ymax = houston_bounds[4])
  locs_houston <- st_crop(locs_tx, box)

  houston_map_spp <- ggplot() +
    geom_spatraster_rgb(data = houston, maxcell = Inf) +
    geom_sf(data = locs_houston,
            aes(size = nspp),
            show.legend = "point",
            color = "blue",
            alpha = 0.5) +
    scale_size_continuous(range = c(2, 5)) +
    labs(size = "No. species") +
    coord_sf(crs = 3857, expand = FALSE, ylim = st_bbox(houston)[c(2, 4)]) +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank())
  
  houston_map_rec <- ggplot() +
    geom_spatraster_rgb(data = houston, maxcell = Inf) +
    geom_sf(data = locs_houston,
            aes(size = nrecords),
            show.legend = "point",
  ...
  houston_ar <- (houston_bounds[3] - houston_bounds[1]) / (houston_bounds[4] - houston_bounds[2])
  ggsave("ttr-apr2026-figs/TX-houston-map-species.png",
         houston_map_spp,
         height = 6,
         width = (6 * houston_ar) + 0.5,
         units = "in",
         dpi = 600)
  ggsave("ttr-apr2026-figs/TX-houston-map-records.png",
         houston_map_rec,
         height = 6,
         width = (6 * houston_ar) + 0.5,
         units = "in",
...
```{r}
#| label: fig-state4-spp
#| fig-cap: Locations of sites where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 8

knitr::include_graphics("ttr-apr2026-figs/ttr-2026-map-species.png")
```

```{r}
#| label: fig-state4-rec
#| fig-cap: Locations of sites where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 8

knitr::include_graphics("ttr-apr2026-figs/ttr-2026-map-records.png")
```

```{r}
#| label: fig-la-spp
#| fig-cap: Locations of sites in Louisiana where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/LA-map-species.png")
```

```{r}
#| label: fig-la-rec
#| fig-cap: Locations of sites in Louisiana where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/LA-map-records.png")
```

```{r}
#| label: fig-nm-spp
#| fig-cap: Locations of sites in New Mexico where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/NM-map-species.png")
```

```{r}
#| label: fig-nm-rec
#| fig-cap: Locations of sites in New Mexico where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/NM-map-records.png")
```

```{r}
#| label: fig-ok-spp
#| fig-cap: Locations of sites in Oklahoma where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/OK-map-species.png")
```

```{r}
#| label: fig-ok-rec
#| fig-cap: Locations of sites in Oklahoma where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/OK-map-records.png")
```

```{r}
#| label: fig-tx-spp
#| fig-cap: Locations of sites in Texas where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-map-species.png")
```

```{r}
#| label: fig-tx-rec
#| fig-cap: Locations of sites in Texas where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-map-records.png")
```

```{r}
#| label: fig-dallas-spp
#| fig-cap: Locations of sites in the Dallas, TX area where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-Dallas-map-species.png")
```

```{r}
#| label: fig-dallas-rec
#| fig-cap: Locations of sites in the Dallas, TX area where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-Dallas-map-records.png")
```

```{r}
#| label: fig-austin-spp
#| fig-cap: Locations of sites in the Austin, TX area where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-Austin-map-species.png")
```

```{r}
#| label: fig-austin-rec
#| fig-cap: Locations of sites in the Austin, TX area where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-Austin-map-records.png")
```

```{r}
#| label: fig-houston-spp
#| fig-cap: Locations of sites in the Houston, TX area where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of species). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-Houston-map-species.png")
```

```{r}
#| label: fig-houston-rec
#| fig-cap: Locations of sites in the Houston, TX area where priority species were monitored between 1 October 2023 and 26 April 2026 (point size represents number of records). 
#| echo: false
#| fig.width: 10

knitr::include_graphics("ttr-apr2026-figs/TX-Houston-map-records.png")
```

```{r}
#| label: tbl-sites
#| tbl-cap: Summary of sites monitored in each state from 1 October 2023 to 26 April 2026. Here, an observation is all data collected for a plant on a given date and a record is data collected for a single phenophase.
#| echo: false

knitr::kable(locs_by_state,
             col.names = c("State", "No. sites", "No. species", "No. plants", "No. observations", "No. records",
                           "Mean no. species per site", "Mean no. plants per site", 
                           "Mean no. of observations per site", "Mean no. records per site"),
             align = "c")
```
...
plantspp <- plants %>%
  ...
  summarize(nplants = n(),
            ...
            mean_no_records = round(mean(nrecords), 1),
            .groups = "drop") %>%
  data.frame() %>%
...
```{r}
#| label: tbl-plants
#| tbl-cap: Summary of plants monitored from 1 October 2023 to 26 April 2026. Here, an observation is all data collected for a plant on a given date and a record is data collected for a single phenophase.
#| echo: false

knitr::kable(plantspp %>%
               select(common_name, func_type, nplants_LA, nplants_NM, nplants_OK, nplants_TX,
                      nplants, nsites, mean_no_visits, mean_no_records),
             col.names = c("Species", "Functional group", 
                           "No. plants: LA", "No. plants: NM", "No. plants: OK", 
                           "No. plants: TX", "Total no. plants", "No. sites", 
                           "Mean no. observations", "Mean no. records"),
             align = c("r", "r", rep("c", 8)))
```
...
hist1_file <- "ttr-apr2026-figs/ttr-2026-spp-per-observer.png"
...
hist2_file <- "ttr-apr2026-figs/ttr-2026-plants-per-spp.png"
...
```{r}
#| label: tbl-observer
#| tbl-cap: Summary of people who submitted data for priority species between 1 October 2023 and 26 April 2026, by state. Values in table are means, with range of values in parentheses.
#| echo: false
...
#| fig-cap: Number of species each observer monitored between 1 October 2023 and 26 April 2026.
#| echo: false
#| fig.height: 3
#| fig.width: 8
...
#| fig-cap: Number of plants per species that observers monitored between 1 October 2023 and 26 April 2026.
#| echo: false
#| fig.height: 3
#| fig.width: 8
...
To evaluate observation frequency, we downloaded individual phenometrics data for priority species from 1 October 2023 through 26 April 2026.

```{r download-ip-data}
#| code-fold: true

# Download individual phenometrics data
ip_filename <- "data/ttr-ipdata-2024apr2026.csv"

if (!file.exists(ip_filename) | update_data == TRUE) {
  
  # Download flowering, fruiting data: phenometrics aggregated over water year.
  ...
  # need to use years = 2023:2025 for this call.
...
For these summaries, we evaluated observation frequency by classifying flowering and fruiting observations based on the time elapsed between a "no" observation and the first "yes" observation of a given phenophase. Here, we compared first yeses that were submitted between 1 October 2025 and 26 April 2026 (2026 water year) with first yeses that were submitted in prior water years.

```{r classifying-prior-nos}
#| code-fold: true

# Note that if a first yes occurred very early in the water year, the downloaded data may not report that
# there was a prior no shortly before. Number of days since prior no may be NA, even if there was a "no" 
# observation not long before if it occurred in a different water year. Ultimately, it would be better
# if we used status data to create first yeses that could better account for observations around the
# start of the water year, but not going to bother with that now. 

ip <- ip %>%
  # Create season (ie, water year, where Oct 2024 - Sept 2025 = 2025)
  mutate(yes_date = parse_date_time(paste(first_yes_year, first_yes_doy),
                                    orders = "Y j"),
         yes_month = month(yes_date),
         season = case_when(
           yes_month %in% 10:12 ~ first_yes_year + 1,
...
spp20 <- unique(obsfreq_spp$common_name[obsfreq_spp$nobs >= 20])
obsfreq_sppl <- ip %>%
  filter(!is.na(season)) %>%
  filter(php != "leaves") %>%
  filter(common_name %in% spp20) %>%
...
#| fig-cap: Proportion of first "yes" observations that were associated with a prior "no" with a given number of days, grouped by phenophase. 2024 = October 2023 - September 2024; 2025 = October 2024 - September 2025; and 2026 = October 2025 - 26 April 2026.
#| echo: false
#| fig.height: 5
#| fig.width: 10
...
#| fig-cap: Proportion of first "yes" observations that were associated with a prior "no" with a given number of days, grouped by species. We excluded species with fewer than 20 "yes" observations in both water years. 2024 = October 2023 - September 2024; 2025 = October 2024 - September 2025; and 2026 = October 2025 - 26 April 2026.
#| echo: false
#| fig.height: 5
#| fig.width: 10
...
plant_info2 %>%
  kable(col.names = c("Species", "No. plants", "Planting date", "Death date", 
                      "Patch (size)", "Wild (Y/N/Unreported)", 
                      "Watered (Y/N/Unreported)", "Fertilized (Y/N/Unreported)"),
        align = c("l", rep("c", 7))) %>%
  kableExtra::kable_styling() %>%
  row_spec(nrow(plant_info2), bold = TRUE, hline_after = TRUE)

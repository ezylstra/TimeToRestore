...
```{r}
#| label: intensity-no-postive-status
#| tbl-cap: Observations with an intensity value, where phenophase state was not positive.

knitr::kable(int_no %>% 
               select(site_name, state, common_name, php, phenophase_status, intensity_value,
                      obsdate),
             col.names = c("Site name", "State", "Species", "Phenophase", "Status", "Intensity",
                           "Observation date"))
```
...
knitr::kable(intensitycat_high %>%
             ...
             col.names = c("Species", "Phenophase", "Intensity category", "No. 2024 obs", 
                           "No. 2024 plants", "No. 2025 obs", "No. 2025 plants"),
...
if (!file.exists(map_file) | update_data == TRUE) {
...
![Locations of sites in Louisiana where priority species were monitored between October 2024 and present.](ttr-2025-figs/LA-map.png)

![Locations of sites in New Mexico where priority species were monitored between October 2024 and present.](ttr-2025-figs/NM-map.png)

![Locations of sites in Oklahoma where priority species were monitored between October 2024 and present.](ttr-2025-figs/OK-map.png)

![Locations of sites in Texas where priority species were monitored between October 2024 and present.](ttr-2025-figs/TX-map.png)
